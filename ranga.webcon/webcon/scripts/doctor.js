...
doctor.results = {
	...
	ISP_PSK_DENIED: ["用户的用户名或密码错误", "对方在 RFC1334 的某种协议中返回了未知且不可读的信息。无标准互联网规范保证实施一致，根据我们的使用经验猜测，这可能是由于你的用户名或密码错误导致的。", "检查你的用户名和密码是否正确|检查是否 ISP 出现了问题，请尝试在计算机上直接使用 ISP 的拨号工具进行拨号检查能否拨号，如果失败请联系 ISP 工单客服解决问题"],
	...
	ISP_MULTIPLE_DENIED: ["重复登录/拒绝单号多拨", "对方在 RFC1334 的某种协议中返回了未知且不可读的信息。无标准互联网规范保证实施一致，根据我们的使用经验猜测，这可能是由于你尝试在单一端口上尝试重复连接单一账户。也可能是你的帐号已经在另一处登录，或者在其他终端没有正常退出", "在其他终端断开连接|尝试在 ISP 提供的网页上使用“强制下线”功能|检查接口的认证信息|检查系统日志获取信息|检查是否 ISP 出现了问题，请尝试在计算机上直接使用 ISP 的拨号工具进行拨号检查能否拨号，如果失败请联系 ISP 工单客服解决问题"],
